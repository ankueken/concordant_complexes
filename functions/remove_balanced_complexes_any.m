function [model_new,status] = remove_balanced_complexes_any(model,B)
% function to remove a set of given balanced complexes applicable for any
% kinetics
%
% [model_new,status] = remove_balanced_complexes_any(model,B)
%
% Input
...
%     model_new.A(id,:) = [];
    ...
    %% model rewriting
    % internal complex
    if ~isempty(P.id) && ~isempty(S.id) && length(out_rxns)==1 && length(in_rxns)>=1
...
%         S.stoich(id,:) = [];
    ...
    elseif isempty(P.id) && ~isempty(S.id) && length(out_rxns)==1 && length(in_rxns)>=1
...
%         P.stoich(id,:) = [];
    ...
    elseif length(out_rxns)==1 && length(in_rxns)>=1
...
%     c_remove=find(all(model_new.A'==0));
    ...
    while ~isempty(r_remove) || ~isempty(m_remove)
