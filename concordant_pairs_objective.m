...
% irreversibility and optimality constraints considered

% To run the code please adapt Line 25 -> path to R (third argument)
...
for f_n=1:length(species_list)
    ...
    species_degree_complexes = sum(Results_balanced.MODEL_o{1}.Y~=0,2);
    species_degree_reactions = sum(Results_balanced.MODEL_o{1}.S~=0,2);

    %% balanced complexes
    [B,group] = find_balanced_complexes(Results_balanced.MODEL_o{1},1e-6);
...
% 
    CC = zeros(length(Results_balanced.MODEL_o{1}.complexes));
    CC(B,B) = 1;
% 
...
% 
    ...
    for i=1:length(idx)
        tcc = find(Results_balanced.MODEL_o{1}.Y(idx(i),:)~=0);
        if CC(tcc,tcc)~=1
            CC(tcc,tcc) = -1;
    ...
    coupling_pairs = find_coupling_complexes(Results_balanced.MODEL_o{1},B,f_n,0.99,group);
...
% *Reaction pattern around concordant complexes*
    ...
    for i=1:size(coupling_pairs,1)
        reactions_concordant_complexes.Ci_in(i,1) = sum(Results_balanced.MODEL_o{1}.A(coupling_pairs(i,1),:)>0);
        reactions_concordant_complexes.Ci_out(i,1) = sum(Results_balanced.MODEL_o{1}.A(coupling_pairs(i,1),:)<0);
        reactions_concordant_complexes.Cj_in(i,1) = sum(Results_balanced.MODEL_o{1}.A(coupling_pairs(i,2),:)>0);
        reactions_concordant_complexes.Cj_out(i,1) = sum(Results_balanced.MODEL_o{1}.A(coupling_pairs(i,2),:)<0);
    end
...
% *Concordance modules*
   ...
   unclassified = unique(CP);
    ...
    save(strcat('Results_concordant_original_objective/',name,'.mat'))
end